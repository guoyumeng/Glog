	...
	<div>
		...
		<el-table
			...
			<el-table-column
		        ...
		        size="mini"
				  	type="primary"
						@click="handleEdit(props.$index, props.row)">更新</el-button>
				  <el-button
				  	size="mini"
				  	type="warning"
    ...
    data() {
      ...
      handleCurrentChange(val) {
			...
			getList(){
				...
				var data = new FormData();
				data.append("name",this.search_input);
				this.axios.post(that._path.php_path+'/php/getDataNum.php',data).then(res=>{
			...
			handleStop(index, row) {
				...
				data.append("state",1);
				this.axios.post(that._path.php_path+"/php/state.php",data).then(res=>{
					this.$message({
						type: 'success',
						message: '停用成功!'
					});
          ...
          type: 'error'
					...
					this.axios.post(that._path.php_path+"/php/deluser.php",data).then(res=>{
						this.$message({
							type: 'success',
							message: '删除成功!'
						});
